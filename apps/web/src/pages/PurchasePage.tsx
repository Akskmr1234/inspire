...
export function PurchasePage(): React.JSX.Element {
  ...
  const columns: readonly GridColumn<PurchaseInvoiceSummary>[] = [
    { key: 'number', header: t('purchase.number'), value: (row) => row.number },
    {
      ...
      value: (row) => (isPurchaseReturn(row.kind) ? t('purchase.return') : t('purchase.invoice')),
      render: (row) => (
        <span
          className={clsx(
    ...
    { key: 'lines', header: t('purchase.lines'), value: (row) => row.lineCount, numeric: true },
    { key: 'taxable', header: t('purchase.taxable'), value: (row) => row.taxable, numeric: true },
    { key: 'tax', header: t('purchase.tax'), value: (row) => row.tax, numeric: true },
    { key: 'total', header: t('purchase.total'), value: (row) => row.total, numeric: true },
    {
      key: 'status',
...
function EntryDialog({
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      ...
      const header = await createPurchaseInvoice({
        ...
        lines: payload,
        kind,
        returnsInvoiceId: isPurchaseReturn(kind) && returnsInvoiceId ? returnsInvoiceId : null,
        ...
        supplierInvoiceDate: supplierInvoiceNumber.trim() ? supplierInvoiceDate || null : null,
      });
  ...
  return (
    ...
    >
      ...
      <p className="rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-600 dark:bg-slate-900 dark:text-slate-400">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <span className="text-sm">
          {t('purchase.tax')}: <strong className="font-mono">{totals.tax.toFixed(2)}</strong>
        </span>
        <span className="text-base">
          {t('purchase.total')}: <strong className="font-mono">{totals.total.toFixed(2)}</strong>
        </span>
      </div>
      ...
      <p className="text-xs text-slate-500">{t('purchase.totalsHint')}</p>
              ...
              {needsSerials && (
                ...
                >
                  <textarea
                    ...
                    className="w-72 rounded-md border border-slate-300 bg-white px-2 py-1 font-mono text-xs dark:border-slate-700 dark:bg-slate-900"
                  />
                </Field>
              )}
            </div>
      ...
      {document && (
        <>
          <div className="grid gap-2 text-sm sm:grid-cols-3">
            ...
            <Detail label={t('purchase.status')} value={statusLabel(document.header.status, t)} />
            ...
            />
            <Detail label={t('purchase.taxable')} value={document.header.taxable.toFixed(2)} />
            <Detail label={t('purchase.tax')} value={document.header.tax.toFixed(2)} />
            <Detail label={t('purchase.total')} value={document.header.total.toFixed(2)} />
          </div>
            ...
            <tbody>
              {document.lines.map((line) => (
                ...
                >
                  ...
                  <td className="px-2 py-1 font-mono text-xs">{line.batchNumber ?? '—'}</td>
          ...
          {document.stockDocumentId && (
            <p className="text-xs text-slate-500">{t('purchase.postedProduced')}</p>
          )}
...
function Select<TValue extends string | number>({
  ...
  return (
    <select
      ...
      onChange={(event) =>
        onChange(
          (typeof value === 'number' ? Number(event.target.value) : event.target.value) as TValue,
        )
      }
      className="rounded-md border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-900"
    >
