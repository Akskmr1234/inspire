...
| Inventory — per-firm account map for stock postings (answer to Q8a) | **Done** — 7 domain tests, seeded per firm; the posting itself is next |
| Sales, Purchase, Manufacturing, Service modules | Not started |
| Keycloak / SSO (deferred by request — plain JWT in its place) | Deferred |

**Test suite:** 909 passing, 0 failing (468 domain + 206 application + 152 API + 20 identity + 63 integration).
