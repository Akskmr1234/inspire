...
public sealed record BounceChequeCommand(
    Guid ChequeId,
    string Reason,
    DateOnly BouncedOn) : ICommand<BouncedChequeResponse>, ITransactional;
...
/// Always <see langword="true"/>. The bills are put back automatically; the ledger
/// postings are not, and the caller must say so.
/// </param>
...
/// </para>
/// <para>
/// The ledger postings are a different matter and are deliberately not reversed
/// automatically. Which control account a bounced cheque comes back out of, and where
/// the bank's charge for it goes, are a firm's own choice of chart; inventing an
/// answer would mean posting into somebody's books on a guess. The response says
/// plainly that a reversing entry is still owed, so a caller cannot mistake silence
/// for completeness.
/// </para>
/// </remarks>
    ...
    : ICommandHandler<BounceChequeCommand, BouncedChequeResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        Cheque cheque = found.Value;
        Result bounced = cheque.Bounce(request.Reason, request.BouncedOn);
        ...
        return Result.Success(new BouncedChequeResponse(
            ChequeContext.StateOf(cheque), reopened, total, LedgerReversalRequired: true));
    }
